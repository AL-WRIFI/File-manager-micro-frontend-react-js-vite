...
export const deleteFolder = (folder) => async (dispatch) => {
  ...
  const deleteFolderAndSubfolders = async (folderRef, batch) => {
    try {
      ...
      if (snapshot.exists) {
        ...
        subfoldersArray.forEach(async (subfolderId) => {
          ...
          await deleteFolderAndSubfolders(subfolderRef, subfolderBatch);
  ...
  try {
    await deleteFolderAndSubfolders(folderRef, mainBatch);
